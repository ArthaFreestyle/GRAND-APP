...
/**
 * How much of the active catalogue can actually be judged healthy or not, and
 * how big that catalogue is.
 *
 * This exists for Beranda's stock-health score, which is a ratio, and whose
 * denominator was wrong in a way that made the card read 100 / 100 for
 * practically every real shop. Two things were going on at once, and either
 * alone was enough to break it:
 *
 *  1. **A product with no reorder point was being counted as healthy.**
 *     `GET /product/stok-minimum` can only ever answer with products that have
 *     one — `stok_minimum = 0` is the column default, meaning "belum diatur",
 *     and the endpoint documents that it never returns those rows. Dividing by
 *     every active product therefore scores an unconfigured catalogue 100,
 *     which is the highest possible mark for having answered nothing.
 *  2. **The ratio could not move.** One item below minimum in a four-hundred
 *     item catalogue is 99.75, and `Math.round` reports that as 100. It took
 *     roughly half a percent of the whole catalogue going low before the
 *     number changed at all.
 *
 * So the denominator has to be the set the question can be asked of: active
 * products that have a reorder point. **There is no filter for that** — `GET
 * /product` takes only `page`, `size`, `search` and `is_aktif` — so it is
 * counted by walking the catalogue at the contract's maximum page size. The
 * list payload does carry `stok_minimum` (it is `satuan` and `harga_jual` that
 * are detail-only, and stock that is absent entirely), so the walk is the only
 * cost: no per-row read, and nothing of the N+1 the contract warns against.
 *
 * **The walk is capped, and `lengkap` is how the caller finds out.** Past the
 * cap `berambang` is a floor rather than a count, and a score computed from a
 * floor is an invented number — exactly what the layout-economy rule forbids,
 * so the caller draws no score at all instead. Page one goes alone because it
 * is what says how many pages there are; the rest go together.
 */
export interface AmbangStok {
  /** Active products with `stok_minimum > 0` — the only ones a score can judge. */
  berambang: number;
  /** Every active product, from `paging.total_item`. */
  aktif: number;
  /** False when the catalogue outran the page cap, so `berambang` is a floor. */
  lengkap: boolean;
}

/** The contract's maximum `size`. Fewer round trips per product counted. */
const AMBANG_SIZE = 100;
/**
 * Ten pages, so a thousand active products. Beranda fires this on launch — every
 * native tab mounts eagerly — so the ceiling is what keeps a cold start from
 * turning into an unbounded scan on a catalogue nobody has sized yet.
 */
const AMBANG_PAGE_CAP = 10;

export async function hitungAmbangStok(cap = AMBANG_PAGE_CAP): Promise<AmbangStok> {
  const hitung = (rows: ProductRow[]) => rows.filter((r) => r.stokMin > 0).length;

  const first = await listProducts({ page: 1, size: AMBANG_SIZE, is_aktif: true });
  const aktif = first.paging.total_item ?? first.data.length;
  const totalPage = Math.max(1, first.paging.total_page ?? 1);
  let berambang = hitung(first.data);

  if (totalPage > 1) {
    const last = Math.min(totalPage, cap);
    const rest = await Promise.all(
      Array.from({ length: last - 1 }, (_, i) =>
        listProducts({ page: i + 2, size: AMBANG_SIZE, is_aktif: true })
      )
    );
    for (const p of rest) berambang += hitung(p.data);
  }

  return { berambang, aktif, lengkap: totalPage <= cap };
}
