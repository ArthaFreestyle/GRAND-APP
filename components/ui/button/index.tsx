// @ts-nocheck — vendored gluestack-ui v5 alpha component.
//
// Generated by `npx gluestack-ui add`, and regenerated wholesale by
// `gluestack-ui upgrade`, so edits here do not survive. It does not typecheck
// against nativewind 4.2.6: its `cssInterop` mapping passes `true` for style
// keys that are not props on the icon component, which this version's
// `EnableCssInteropOptions` rejects. Runtime is unaffected — Metro strips types
// and the bundle is correct.
//
// Suppression is file-scoped on purpose: our own code that *uses* these
// components is still fully checked, and the exported types still flow.
'use client';
