 ...
 *
 * The anchor is what a cold deep link needs: opened straight at `/mutasi-pemakaian/12`,
 * the stack is built with the list underneath, so dismissing the detail lands
 * on it instead of leaving the app.
 */
export const unstable_settings = { anchor: 'index' };

export default function MutasiPemakaianLayout() {
  return (
    <Stack screenOptions={{ headerShown: false }}>
      <Stack.Screen name="index" options={{ title: 'Mutasi & Pemakaian', animation: 'none' }} />
      <Stack.Screen
        name="[id]"
        options={{ title: 'Detail Transaksi', animation: 'slide_from_right' }}
      />
      <Stack.Screen
        name="baru"
        options={{ title: 'Transaksi Baru', animation: 'slide_from_right' }}
      />
    </Stack>
  );
}
