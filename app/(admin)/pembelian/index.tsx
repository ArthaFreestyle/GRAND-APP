/**
 * Pembelian — the list of purchase invoices.
 *
 * The invoice detail and the entry form are routes (`[id]` and `baru`), so this
 * screen keeps its search, its filter, and its page while either is on top; the
 * seeded dataset both of them write to lives in `stores/pembelian.ts`.
 */
    ...
    <AppShell title="Pembelian">
      <View style={styles.wrap}>
        <View style={styles.toolbar}>
          ...
          {canWrite && (
            <PrimaryButton label="Faktur baru" onPress={() => router.push('/pembelian/baru')} />
          )}
        </View>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
          <StatTile
            ...
            sub={`${openList.length} faktur belum lunas`}
          />
          <StatTile
            label="Jatuh tempo terlewat"
            value={rp(sumOverdue)}
            valueClass={sumOverdue > 0 ? C.red : C.text}
            sub={`${overdueList.length} faktur lewat tempo`}
          />
        ...
        <DataTable
          ...
          {slice.map((f) => {
            ...
            return (
              <Pressable
                key={f.id}
                onPress={() => router.push({ pathname: '/pembelian/[id]', params: { id: f.id } })}
                style={styles.row}>
          ...
          {slice.length === 0 && (
            <EmptyState
              title="Tidak ada faktur yang cocok"
